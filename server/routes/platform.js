...
export function registerPlatformRoutes(app) {
  ...
  app.put('/api/settings', (req, res) => {
    ...
    if (b.notifications?.telegram?.chatId && !/^-?\d+$/.test(String(b.notifications.telegram.chatId).trim())) {
      return res.status(400).json({ error: 'The Telegram chat ID should be a number (e.g. 123456789 or -1001234567890 for a group)' });
    }
