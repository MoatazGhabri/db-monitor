...
const REQUIRED = {
  google_drive: ['folderId', 'serviceAccountJson'],
  local: ['path'],
  webdav: ['url'],
...
function validateDrive(provider, config) {
  ...
  if (provider === 'google_drive') {
    try {
      ...
      if (!sa.client_email || !sa.private_key) return 'This does not look like a Google service account key — it should contain "client_email" and "private_key".';
    } catch {
      return 'The service account key is not valid JSON. Paste the full contents of the .json file Google gave you.';
    }
  }
  return null;
}
