...
function NotificationsTab({ settings, saving, onSave }: { settings: Settings; saving: boolean; onSave: (p: Partial<Settings>) => void }) {
  ...
  const test = async (channel: 'slack' | 'webhook', url: string) => {
    setTesting(channel);
    setTestMsg(null);
  ...
  return (
    <div className="space-y-3">
      {[{ key: 'slack' as const, label: 'Slack', value: slack, set: setSlack, placeholder: 'https://hooks.slack.com/services/…' },
        ...
        <Card key={ch.key}>
          ...
          <div className="p-5 space-y-3">
            ...
            <Button variant="secondary" size="sm" icon={testing === ch.key ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />} disabled={!ch.value.url || testing !== null} onClick={() => test(ch.key, ch.value.url)}>Send test</Button>
          </div>
        </Card>
      ))}
      ...
      <Button variant="primary" icon={saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />} onClick={() => onSave({ notifications: { slack, webhook, events } })}>Save</Button>
    </div>
  );
}
