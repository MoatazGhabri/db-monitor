...
export interface TelegramChannel { enabled: boolean; botToken: string; chatId: string; }
export interface Settings {
  ...
  notifications: {
    ...
    events: { connectionDown: boolean; backupFailed: boolean; backupSucceeded: boolean; diskFull: boolean };
  };
...
export async function testNotification(channel: 'telegram', params: { botToken: string; chatId: string }): Promise<{ success: boolean }>;
export async function testNotification(channel: 'slack' | 'webhook', params: { url: string }): Promise<{ success: boolean }>;
export async function testNotification(channel: 'telegram' | 'slack' | 'webhook', params: Record<string, string>): Promise<{ success: boolean }> {
  requireLocal('Notifications');
  return localPost('/settings/test-notification', { channel, ...params });
}
...
export type DriveProvider = 'google_drive' | 'local' | 'webdav' | 's3';
...
export async function fetchSavedQueries(): Promise<{ queries: SavedQuery[] }> {
  requireLocal('Saved queries');
  return localGet('/saved-queries');
}
export async function createSavedQuery(input: { name: string; sql: string; connectionId?: string | null }): Promise<{ query: SavedQuery }> {
  requireLocal('Saved queries');
  return localPost('/saved-queries', input);
}
